...
public class FlagActivity extends AppCompatActivity implements View.OnTouchListener {
  ...
  //  private int flagSet=0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        minesnum = (TextView)findViewById(R.id.mineView);
        minesnumcount = Integer.parseInt(MainActivity.allBoomsCount);
        Intent intent=getIntent();
       // flagSet=Integer.parseInt(intent.getStringExtra("flagset"));
        gv=(GridView)findViewById(R.id.gv);
        gv.setOnTouchListener(this);
        level = Integer.parseInt(MainActivity.level);
        adapter=new BoomAdapter(level,Integer.parseInt(MainActivity.flagset),gv,this);
    ...
    public void startGame(){
        minesnum.setText("Mines: "+MainActivity.allBoomsCount);
        minesnumcount = Integer.parseInt(MainActivity.allBoomsCount);
        adapter=new BoomAdapter(level,Integer.parseInt(MainActivity.flagset),gv,this);
        gv.setNumColumns(level);
        gv.setAdapter(adapter);
        ...
        origin=random.nextInt(level*level/2)+2;
        while (origin<level*level) {
            GridEntity grid = adapter.getItem(origin);
            if (grid.getBoomsCount()==0&&!grid.isBoom()){
                adapter.showNotBoomsArea(origin);
                grid.setIsShow(true);
                break;
            }
            origin ++;
...
//
                    ...
                    values.put("mode", "Flags");
                    values.put("Time", gameTime);
                    HelperActivity helpter = new HelperActivity(FlagActivity.this);
                    helpter.insert(values);
                    return true;
                }
                adapter.notifyDataSetChanged();
                if(adapter.isWinflag()){
                    mp_win.start();
                    stopGame();
